...
-- h (Harpoon)
	...
	map('n', '<leader>0', '<CMD>lua HarpoonPrint()<CR>', opts)
	map('n', '<leader>hD', '<CMD>lua HarpoonDeleteAll()<CR>', opts)
	for _, value in pairs(keybinds) do
		map('n', '<leader>' .. value, '<CMD>lua HarpoonSwitch(' .. value .. ')<CR>', opts)
		map('n', '<leader>h' .. value, '<CMD>lua HarpoonDefine(' .. value .. ')<CR>', opts)
		map('n', '<leader>hd' .. value, '<CMD>lua HarpoonDelete(' .. value .. ')<CR>', opts)
	end
-- i (Inspect)
	map('n', '<leader>i', '<CMD>Inspect<CR>', nrmp)
-- l (LaTeX)
	map('n', '<leader>lc', '<CMD>lua LatexCompile()<CR>', nrmp)
	map('n', '<leader>lx', '<CMD>lua LatexClean()<CR>', nrmp)
	map('n', '<leader>lv', '<CMD>lua LatexView()<CR>', nrmp)
	map('n', '<leader>ls', '<CMD>lua SymPy()<CR>', nrmp)
...
-- s (Search Telescope)
	...
	map('n', '<leader>sp', '<CMD>lua ToggleNetRW()<CR>', opts)
...
-- t (Toggle)
	...
	map('n', '<leader>tf', '<CMD>lua ToggleFoldcolumn()<CR>', opts)
	...
	map('n', '<leader>tl', '<CMD>lua ToggleLanguage()<CR>', opts)
